...
export default function Creative({ editor }: creativeProps) {
  ...
  //         if (index === inputIndex) {
  //           imageObj.borderColor === "black"
  //             ? (imageObj.borderColor = "blue")
  //             : (imageObj.borderColor = "black");
  //         }
  //         return imageObj;
  //       })
  ...
  const onClickImageUpload = async () => {
    ...
    editor?.addImage(downloadUrl);

    // setImageObjs([
    //   ...imageObjs,
    //   { imageUrl: downloadUrl, borderColor: "black" },
    // ]);
  };
      ...
      {/* {imageObjs.map((imageObj, index) => {
        return (
          <Image
            ...
            onClick={() => handleImageClick(Number(index))}
          ></Image>
        );
      })} */}
